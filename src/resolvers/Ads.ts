...
export function getAdQueryWhere(graphqlWhere?: AdsWhere):{[key: string]: unknown;} {
...
export class AdsResolver {
  ...
  async allAds(
   ...
   Promise<Ad[]> {
    ...
    const ads = await Ad.find({take: take ?? 50, skip, where: queryWhere, relations: {
        category: true,
        tags: true,
        createdBy: true,
      },
  ...
  async allAdsCount(@Arg("where", { nullable: true }) where?: AdsWhere): Promise<number> {
    const queryWhere = getAdQueryWhere(where);
    const count = await Ad.count({where: queryWhere,});
  ...
  async ad(@Arg("id", () => ID) id: number): Promise<Ad | null> {
    const ad = await Ad.findOne({where: { id: id }, relations: { category: true, tags: true },});
  ...
  @Authorized()
  @Mutation(() => Ad)
  async createAd(@Ctx() context: ContextType,@Arg("data", () => AdCreateInput) data: AdCreateInput):Promise<Ad> {
    const newAd = new Ad();
  ...
  async updateAd(@Ctx() context: ContextType, @Arg("id", () => ID) id: number, @Arg("data") data: AdUpdateInput):
   Promise<Ad | null> {
    const ad = await Ad.findOne({where: { id: id }, relations: { tags: true, createdBy: true },});
